...
window.addEventListener('load', ()=>{
    for (let i = 1; i < localStorage.length; i++) {
        ...
        task_text.textContent = `${localStorage.key(iz)}`;
        ul.appendChild(li);
...
add.addEventListener('click', () => {
    ...
    let li = document.createElement('li');
    localStorage.setItem(`${task_idx}` , `${task_Value.value}`);
    let local_value =  localStorage.getItem(`${task_idx}` , `${task_Value.value}`)
    console.log(local_value);
    li.setAttribute('id', `${task_idx}`)
    console.log(li.id);

    let task_text = document.createElement('span');
    task_text.textContent = local_value;
    li.appendChild(task_text)
